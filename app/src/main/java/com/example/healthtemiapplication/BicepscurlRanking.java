...
public class BicepscurlRanking extends AppCompatActivity {
    BarChart barChart;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_bicepscurl_ranking);
        ...
        CreateChart(UserList.userList.get(User.rootUser).healthDatas,"biceps");
    }
    public void CreateChart(Map<String, HealthData> healthDatas, String exercise){
        ...
        switch (exercise){
            case "biceps":
                for (String key :
                        xLabel) {
                    sets.add(new BarEntry(i, healthDatas.get(key).getBiceps().getSets()));
                    rests.add(new BarEntry(i, healthDatas.get(key).getBiceps().getRest_time()));
                    works.add(new BarEntry(i, healthDatas.get(key).getBiceps().getWork_time()));
                    i++;
                }
                break;
            case "benchpress":
